...
private const val PY_FUNCTION_PATTERN =
    "(?<=[\\s|\\.])([a-zA-Z]+[a-zA-Z_0-9]*)(?=\\()|^([a-zA-Z]+[a-zA-Z_0-9]*)(?=\\()"

private const val PY_VALUE_PATTERN = "(?<=[\b|\\s])(True|False|None)(?=[\b|\\s])"

private const val PY_COMMENT_PATTERN = "^(#.*)(?=[\r\n])|^(#.*)$"
...
object LangPython : Lang {
    ...
    override val langConfig by lazy {
        mapOf<Regex, @ColorRes Int>(
            ...
            Regex(PY_COMMENT_PATTERN) to R.color.comment_highlight
        )
    }
}